...
import { CheckCircle2, Loader2, AlertCircle, Compass } from 'lucide-react';
...
function LoginContent() {
    ...
    useEffect(() => {
        ...
        // Listen for session to be fully ready
    ...
    }, [isLimitReached, supabase.auth]);

    // Handle redirection only when session is confirmed
    useEffect(() => {
        if (sessionReady && success) {
            router.push(next);
        }
    }, [sessionReady, success, next, router]);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = (mode === 'signin'
                ? await signInWithEmail(formData, next)
                : await signUpWithEmail(formData, next)) as any;
            ...
            if (res?.error) {
                ...
                // We don't setLoading(false) here because we want to keep the pulse/loading state until router.push triggers
            }
        } catch (err: any) {
    ...
    const handleGoogleLogin = async () => {
        ...
        setError('');
        try {
            const res = await signInWithGoogle(next) as any;
            if (res?.url) {
                // Hard redirect for OAuth
            ...
            // Only set error if it's NOT a redirect error (though we've removed it)
            if (err.message !== 'NEXT_REDIRECT') {
                ...
                <div className="relative z-10 p-16 max-w-xl space-y-12">
                    ...
                    <div className="space-y-8">
                        ...
                        <p className="text-white/80 text-xl font-medium leading-relaxed max-w-md">
                            Get structured insights to master technical complexity.
                        </p>
                    </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    {mode === 'signin' && (
                                        <a href="#" className="text-xs font-bold text-[#FF7D29] hover:underline transition-all">
                                            Forgot?
                                        </a>
                                    )}
                                </div>
